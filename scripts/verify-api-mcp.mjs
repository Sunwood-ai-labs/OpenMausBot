...
const out = new URL(`../docs/verification/evidence/api-mcp-20260913/${live ? 'live/' : ''}`, import.meta.url);
...
assert.ok(JSON.stringify(instances).includes(live ? 'Live Codex verification' : 'Verification fixture'), 'Only target the isolated verification fixture');
...
try {
  ...
  if (live) {
    const model = instances.instances.find(i=>i.instanceId==='codex').models.default;
    for (const bot of [a,b]) {
      await tool('set_bot_model', { bot_id: bot.id, instance_id:'codex', model, effort:'low' });
      bot.modelSelection = {instanceId:'codex',model,effort:'low'};
    }
  }
  ...
  await api(`/api/bots/${a.id}/messages`, { text: '[API-DM-01] APIからの個別会話です。受信確認をお願いします。', threadId: a.threadId });
  await wait('bot', a);
  await tool('send_bot_message', { bot_id: a.id, task_id: a.threadId, text: '[MCP-DM-02] 同じ会話へMCPから追記しました。' });
  await wait('bot', a);
  const dm = await tool('get_bot_messages', { bot_id: a.id, limit: 20 });
  await api(`/api/groups/${room.id}/messages`, { text: '[API-ROOM-01] 会議を開始します。API連携の確認事項を共有してください。', threadId: room.threadId });
  await wait('channel', room);
  await tool('send_channel_message', { channel_id: room.id, task_id: room.threadId, text: `[MCP-ROOM-02] @${b.name} MCPから会議に参加しました。担当を引き継いでください。` });
  await wait('channel', room);
  ...
  writeFileSync(new URL('summary.json', out), JSON.stringify({ passed: true, model: live ? 'live Codex' : 'scripted fake Claude', a, b, room, dm, group }, null, 2));
